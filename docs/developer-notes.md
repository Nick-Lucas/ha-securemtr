...
The SecureMTR energy sensors no longer advertise a statistics state class, so Home Assistant does not synthesise device-class-based statistics. The Energy Dashboard therefore consumes only the anchored samples provided via `_submit_statistics()`.
