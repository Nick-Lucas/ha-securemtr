...
def test_safe_anchor_datetime_clamps_to_end_of_day_with_seconds() -> None:
    """safe_anchor_datetime should clamp when seconds push the anchor over the boundary."""
...
def test_calibrate_energy_scale_accepts_ratio_within_tolerance() -> None:
    """calibrate_energy_scale should accept ratios close to ln(10)."""

    fudge = LN10 * 1.1
    rows = [{"energy": 1.0, "runtime": (60 * LN10) / fudge}]
    calibration = calibrate_energy_scale(rows, "energy", "runtime", fallback_power_kw=3.0)
    assert calibration.use_scale is True
    assert calibration.scale == pytest.approx(LN10)
    assert calibration.source == "device_scaled"
...
def test_calibrate_energy_scale_respects_custom_tolerance() -> None:
    """calibrate_energy_scale should honour the provided tolerance threshold."""
